...
switch (_type) do {
	...
	case 1: {
		_ids set [0, playSound3D ["z\RegalisSCP_CB\addons\anomaly\data\SCP173\GroundScrapping_start.ogg", _parent, false, getPosASL _parent, 1.4, 1, 45]];
	};
	case 2: {
		_path = format ["z\RegalisSCP_CB\addons\anomaly\data\SCP173\GroundScrapping_idle_0%1.ogg", selectRandom [1,2,3,4]];
		_ids set [1, playSound3D [_path, _parent, false, getPosASL _parent, 1.2, 1, 60]];
	};
	case 3: {
		playSound3D ["z\RegalisSCP_CB\addons\anomaly\data\SCP173\Neck_Snap.ogg", _parent, false, getPosASL _parent, 1.6, 1, 25];
		_needUpdate = false;
	};
};
if (_needUpdate) then {
